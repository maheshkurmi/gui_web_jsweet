	...
	*/
	...
	GuiTimer activeTimer;
	  ...
	  /**
	     ...
	     */
	    public void update(int dt_ms,boolean revalidate) {
	       // setSize();
	       // updateTime(dt_ms);
	        //handleInput();
	        //handleKeyRepeat();
...
public void onTimerTick(GuiTimer timer){
   ...
   * @param tickcount number of times timer ticks -1=infinite, 0=stops timer, 1= runs only once
	...
	public boolean handleMouse(int x, int y,int id,int button, int clickcount,int modifiers){
		...
		*/
		...
		if ((id == AWTMouseEvent.MOUSE_ENTERED || (id == AWTMouseEvent.MOUSE_MOVED))
			...
			boolean controldown=false;//(modifiers & (AWTKeyEvent.CTRL_MASK|AWTKeyEvent.META_MASK)) != 0;
			boolean shiftdown=false;//(modifiers & AWTKeyEvent.SHIFT_MASK) != 0;
	...
	public boolean handleKey(int id, int keycode, int keychar, int modifiers,
		...
		boolean controldown = false;//(modifiers & (AWTKeyEvent.CTRL_MASK|AWTKeyEvent.META_MASK)) != 0;
		boolean shiftdown = false;//(modifiers & AWTKeyEvent.SHIFT_MASK) != 0;
		boolean altdown = false;//(modifiers & AWTKeyEvent.ALT_MASK) != 0;
